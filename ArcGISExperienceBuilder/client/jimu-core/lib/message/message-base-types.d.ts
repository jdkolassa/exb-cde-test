import { type DataRecord, type DataRecordSet, type DataSource } from '../data-sources/data-source-interface';
...
export declare class ExtentChangeMessage implements Message {
    ...
    publishTime: number;
    /**
     * Indicate whether the view is interacive or not.
     * @ignore
     */
    interacive: boolean;
...
/**
 * The `DataSourcesChangeMessage` uses the widget's id `widgetId`, the `MessageType` `DATA_SOURCES_CHANGE` message,
 * the data source's change type `DataSourcesChangeType` (`CREATE` or `REMOVE`), and, the data sources `DataSource[]`.
 * This message is used when needing to implement a workflow upon the creation or removal of some data sources. Other widgets may
 * need to subscribe to this message whenever a workflow involves reliance on data sources change.
 */
